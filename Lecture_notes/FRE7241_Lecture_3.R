...
co_eff <- 2  # multiplier
portf_value <- numeric(n_rows) # create a vector(full of 0) to speed up the calculation afterwards
...
indeks <- which(rutils::diff_it(day_s) < 0) # when move to next month
date_s[head(indeks)]
...
z_scores <- (clos_e - medi_an)/ma_d
z_scores[1:look_back, ] <- 0
tail(z_scores, look_back) # exist an inf in z_scores
range(z_scores)
...
ta_q <- data.table::fread(file="C:/Users/admin/OneDrive - nyu.edu/Fall2021/Algorithmic Portfolio/xlk_tick_trades_2020_03_16.csv")
